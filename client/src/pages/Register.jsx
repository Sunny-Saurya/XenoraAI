    ...
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-400 text-sm mt-2 text-center">Start your journey with AI-powered repo analysis</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2 px-1">Full Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="w-full px-5 py-3.5 bg-black/40 border border-white/10 rounded-xl focus:outline-none focus:border-[var(--color-brand-green)]/50 transition-colors text-white placeholder:text-gray-600"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full neon-btn py-4 flex items-center justify-center gap-2 group mt-4"
          >
            {loading ? (
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          ...
          <Link to="/login" className="text-[var(--color-brand-green)] hover:underline font-medium">
            Sign In
          </Link>
        </div>
      </motion.div>
